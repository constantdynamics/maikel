-- Moria scanner tables: ultra-cheap mining stocks
...
CREATE INDEX IF NOT EXISTS idx_moria_stocks_archived ON moria_stocks(is_archived);
...
-- Enable RLS
ALTER TABLE moria_stocks ENABLE ROW LEVEL SECURITY;
ALTER TABLE moria_scan_logs ENABLE ROW LEVEL SECURITY;

-- Allow service role full access
CREATE POLICY "Service role full access on moria_stocks"
  ON moria_stocks FOR ALL
  USING (true) WITH CHECK (true);

CREATE POLICY "Service role full access on moria_scan_logs"
  ON moria_scan_logs FOR ALL
  USING (true) WITH CHECK (true);

...
export default function DashboardPage() {
  ...
  useEffect(() => {
    function handleKeyDown(e: KeyboardEvent) {
      ...
      const tabKeys: Record<string, ScannerTab> = { '1': 'kuifje', '2': 'zonnebloem', '3': 'biopharma', '4': 'mining', '5': 'hydrogen', '6': 'shipping', '7': 'moria', '8': 'bluepill' };
      if (tabKeys[e.key]) {
                  ...
                  <div className="absolute right-0 mt-2 w-64 bg-[var(--bg-secondary)] border border-[var(--border-color)] rounded-lg shadow-lg z-20 p-3 space-y-3">
                    ...
                    <div>
                      <div className="text-[10px] uppercase tracking-wider text-[var(--text-muted)] font-semibold mb-2">Huidige tab ({
                        { kuifje: 'Kuifje', zonnebloem: 'Zonnebloem', biopharma: 'BioPharma', mining: 'Mining', hydrogen: 'Hydrogen', shipping: 'Shipping', moria: 'Moria', bluepill: 'Blue Pill' }[activeTab]
                      })</div>
        ...
        {activeTab === 'bluepill' && (
          <>
            <div className="flex items-center justify-between flex-wrap gap-4">
              ...
              <div className="flex items-center gap-3">
                <button onClick={handleRunBluepillScan} disabled={bluepillScanRunning}
                  className="px-4 py-2 bg-pink-600 hover:bg-pink-700 disabled:opacity-50 rounded text-sm font-medium text-white transition-colors">
                  {bluepillScanRunning ? 'Scanning...' : 'Run Blue Pill Scan'}
                </button>
              ...
              <div className="flex items-center gap-3 flex-wrap">
                <div className="flex-1 min-w-[200px]">
                  <input type="text" placeholder="Search ticker or company name..."
                    ...
                    className="w-full px-3 py-2 bg-[var(--input-bg)] border border-[var(--border-color)] rounded text-[var(--text-primary)] placeholder-[var(--text-muted)] focus:outline-none focus:border-pink-500 text-sm" />
                </div>
                ...
                {selectedIds.size > 0 && (
                  <div className="flex items-center gap-2 ml-auto">
                    <span className="text-sm text-[var(--text-muted)]">{selectedIds.size} selected</span>
                    <button onClick={() => { const selected = bluepillStocks.filter((s) => selectedIds.has(s.id)); for (const s of selected) window.open(getGoogleFinanceUrl(s.ticker, s.exchange), '_blank'); }}
                      className="px-3 py-2 text-sm bg-pink-600 text-white hover:opacity-90 rounded transition-colors">Open in Google</button>
                    <button onClick={handleBulkFavorite} className="px-3 py-2 text-sm bg-[var(--accent-orange)] text-white hover:opacity-90 rounded transition-colors">Favorite</button>
                    <button onClick={handleBulkArchive} className="px-3 py-2 text-sm bg-blue-600 text-white hover:opacity-90 rounded transition-colors">Archive</button>
                    <button onClick={requestBulkDelete} className="px-3 py-2 text-sm bg-[var(--accent-red)] text-white hover:opacity-90 rounded transition-colors">Delete</button>
                  </div>
            ...
            {bluepillLoading ? (
              ...
              <div className="bg-[var(--card-bg)] border border-[var(--border-color)] rounded-lg p-16 text-center">
                <h2 className="text-xl font-semibold mb-2 text-[var(--text-primary)]">No Blue Pill Stocks Yet</h2>
                <p className="text-[var(--text-secondary)] mb-4">Click &ldquo;Run Blue Pill Scan&rdquo; to find ultra-cheap biopharma stocks that have declined 90%+ from all-time highs.</p>
                <button onClick={handleRunBluepillScan} disabled={bluepillScanRunning}
                  className="px-6 py-3 bg-pink-600 hover:bg-pink-700 disabled:opacity-50 rounded-lg font-medium text-white transition-colors">
                  {bluepillScanRunning ? 'Scanning...' : 'Run First Blue Pill Scan'}
                </button>
              </div>
            ) : (
              <>
